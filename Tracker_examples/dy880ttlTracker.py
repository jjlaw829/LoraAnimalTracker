...
def parse_gps(d, h):
    if d.find('GGA') > 0:
        ...
        if write_to_csv:
            ...
            #print(json.dumps(gps_data)) ;default line 
            return gps_data


# Runs GPS function until signal acquired, then returns gps data as dict
def main(readingGPS = True):
    while readingGPS:
        time.sleep(time_interval)
        try:
            line = ser.readline().decode()
            gps_dict = parse_gps(line, 360.0-sensor.get_bearing())
            
            if gps_dict != None:
                #print(gps_dict.get('datetime'))
                readingGPS = False
                return gps_dict
